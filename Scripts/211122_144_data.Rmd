...
You will need to download your sequences from the Kelp_Remin_fastq folder in the [shared google drive](https://drive.google.com/drive/folders/1kmLhML2Sanv0gE8RnxpcyNEcrhan-Q2h?usp=sharing) to your repository. Because this folder will then contain a lot of data (over the limit for pushing to the GitHub server), you'll want to have your commits ignore the addition and any changes to that folder. THIS IS IMPORTANT. You can modify the .gitignore file in your repository by:
...
path <- "~/Documents/144l_students_2021/Input_Data/week9/Kelp_Remin_fastq" #make sure there is no / at the end of the path
...
```{r fig.height=10, fig.width=12}
plotQualityProfile(fnRs[6:12])
#plotQualityProfile(fnRs[25])
```
...
```{r}
#tutorial says week 5/6, just save in Input & Output data for Week 9 (we are on a different schedule this year than previous)
saveRDS(mergers, "~/Documents/144l_students_2021/Output_Data/week9/144L_dada_merged.rds")
saveRDS(mergers, "~/Documents/144l_students_2021/Input_Data/week9/144L_dada_merged.rds")
```
...
```{r}
taxa <- assignTaxonomy(seqtab.nochim, "~/Documents/144l_students_2021/Input_Data/week9/Reference_Database/silva_nr_v138_train_set.fa", multithread = TRUE)
...
```{r}
saveRDS(t(seqtab.nochim), "~/Documents/144l_students_2021/Output_Data/week9/144L_seqtab-nochimtaxa.rds")
saveRDS(taxa, "~/Documents/144l_students_2021/Output_Data/week9/144L_taxa.rds")

#save in both Output and Input data folders for week7

saveRDS(t(seqtab.nochim), "~/Documents/144l_students_2021/Input_Data/week9/144Lseqtab-nochimtaxa.rds")
saveRDS(taxa, "~/Documents/144l_students_2021/Input_Data/week9/144L_taxa.rds")
